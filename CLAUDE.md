...
- **`vault-backup/`** — automatic git commit+push of the notes folder's repo (e.g. an Obsidian vault). **Planned 2026-07-16, not yet built (0/2 milestones).** Decision reversed mid-planning: first scoped as a separate signed `launchd` helper app, then moved *into* Rapture because Rapture already is the Developer-ID-signed, Full-Disk-Access, continuously-running, both-drive-types app a helper would reconstruct — and its menu bar + Settings are the only way to satisfy the hard "never fail silently" requirement (a headless helper is structurally silent, the exact failure that let obsidian-git and a `launchd` job make zero commits for three days unnoticed). Reuses `DestinationGuard`/`DestinationMonitor`/offline-spool so internal and external drives are one code path. **Adds the app's fourth outbound path** — a `git push` is a `Process` subprocess, invisible to PRIVACY's `grep URLSession\.`, so PRIVACY's claim grows to "two mechanisms, four files" and gains a second grep for the confined `git` invocation; opt-in/off by default, pushes only to the user's own already-configured remote. The `NoiseMeld/second-brain` vault remote was switched HTTPS→SSH on 2026-07-16 to make headless push reliable. Spec: [`_build_plan/vault-backup/prd.md`](_build_plan/vault-backup/prd.md).
