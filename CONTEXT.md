...
- **Output folder** — the user-chosen directory where notes land. Stored as a security-scoped bookmark. Defaults to `~/Documents/Rapture Notes/` on first launch when none configured. Its absolute path is mirrored to `~/Library/Application Support/Rapture for Mac/output-folder.path` so downstream consumers can track folder changes without bookmark knowledge.
