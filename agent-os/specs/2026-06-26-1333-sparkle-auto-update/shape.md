...
- **Updater starts only when a real key is embedded.** `SPUStandardUpdaterController(startingUpdater: true)` treats an invalid/placeholder `SUPublicEDKey` as a **fatal** error at launch — which crashed the test host on CI (the local run masked it: a display server turned the fatal error into a dismissable alert). `UpdaterController` now starts the updater only when `SUPublicEDKey` isn't the placeholder (`isConfigured`); otherwise auto-update stays inert and Settings → About says so. The real EdDSA public key is committed in `set_sparkle_info.sh`, so production/dev builds are configured; the inert path is just a safety net.
